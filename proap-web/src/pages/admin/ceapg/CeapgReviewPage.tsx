...
import useEvaluateCeapg from '../../../hooks/admin/useEvaluateCeapg';
...
const CeapgReviewPage = () => {
  ...
  const tipoDemanda = location.state?.tipoDemanda || 'NORMAL';
  const isExtra = tipoDemanda.toUpperCase() === 'EXTRA';

  const { solicitation, isLoading } = useSolicitation(id, tipoDemanda);
  ...
  if (!solicitation) {
    return (
      <Container maxWidth="lg" sx={{ py: 6, textAlign: 'center' }}>
        <Typography variant="h5" color="text.secondary">Solicitação não encontrada.</Typography>
      </Container>
    );
  }
          ...
          isExtra ? (
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                ...
                <Chip label="Extra" size="small" color="secondary" variant="outlined" sx={{ fontWeight: 600 }} />
              </Box>
                  ...
                  <Grid container spacing={3}>
                    <Grid item xs={12} sm={6}>
                      ...
                      <Typography variant="body1" fontWeight="500">
                        {currentUser.name || solicitation?.user?.name || 'Não informado'}
                      </Typography>
                    </Grid>
                    <Grid item xs={12} sm={6}>
                      <Typography variant="caption" color="text.secondary" display="block">Item Solicitado / Motivo</Typography>
                      <Typography variant="body1">{sData?.itemSolicitado || 'Não informado'}</Typography>
                    </Grid>
                    <Grid item xs={12} sm={6}>
                      <Typography variant="caption" color="text.secondary" display="block">Valor Solicitado</Typography>
                      <Typography variant="h6" fontWeight="700" color="text.primary">
                        {formatNumberToBRL(sData?.valorSolicitado || solicitation.valorTotal || 0)}
                      </Typography>
                    </Grid>
                    <Grid item xs={12} sm={6}>
                      <Typography variant="caption" color="text.secondary" display="block">E-mail de Contato</Typography>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                ...
                <Chip 
                  label={sData?.modalidade || 'Nacional'} 
                  size="small" 
                  color="primary" 
                  variant="outlined" 
                  sx={{ fontWeight: 600 }} 
                ...
                <Grid item xs={12} md={4}>
                  ...
                  <Grid container spacing={2}>
                    <Grid item xs={12}>
                      ...
                      <Typography variant="body1" fontWeight="500">{solicitation.tituloPublicacao || sData?.itemSolicitado || 'Não informado'}</Typography>
                    </Grid>
                    <Grid item xs={12}>
              ...
              <Box>
                ...
                <Grid container spacing={3}>
                  ...
                  <Grid item xs={12} sm={6} md={2}>
                    ...
                    <Typography variant="body1">
                      {solicitation?.dataInicio ? dateToLocalDate(solicitation.dataInicio) : 'Não informada'}
                    </Typography>
                  </Grid>
                  <Grid item xs={12} sm={6} md={2}>
                    <Typography variant="caption" color="text.secondary" display="block">Data de Término</Typography>
                    <Typography variant="body1">
                      {solicitation?.dataFim ? dateToLocalDate(solicitation.dataFim) : 'Não informada'}
                    </Typography>
                  </Grid>
                  <Grid item xs={12} sm={6} md={2}>
              ...
              <Box>
                ...
                <Grid container spacing={3}>
                  ...
                  <Grid item xs={12} sm={3}>
                    ...
                    <Typography variant="body1" fontWeight="500">{solicitation.quantidadeDiariasSolicitadas || 0}x</Typography>
                  </Grid>
                  <Grid item xs={12} sm={3}>
                  ...
                  <Grid item xs={12} sm={3}>
                    ...
                    <Typography variant="h5" fontWeight="700" color="text.primary">
                      {formatNumberToBRL(solicitation.valorTotal || sData?.valorSolicitado || 0)}
                    </Typography>
                  </Grid>
                </Grid>
              </Box>
